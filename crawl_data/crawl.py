...
def scrape_full_database(start_page):
    page = init_browser()
    ...
    try:
        ...
        while True:
            try:
                list_url = f"{BASE_URL}/page/tim-van-ban.aspx?keyword=&area=0&match=True&type=40&status=1&signer=0&sort=1&lan=1&scan=0&org=0&fields=21&chlbg=29/03/1946&chlend=29/03/2036&page={current_page}"
                page.get(list_url)
                ...
                link_elements = page.eles('xpath://p[@class="nqTitle"]//a')
                links = [ele.attr('href') for ele in link_elements if ele.attr('href')]

                if not links:
                    print("Không còn văn bản nào. Hoàn thành!")
                    break
                ...
                for link in links:
                    ...
                    while retry_crawl and attempt < max_retries:
                        try:
                            ...
                            print(f"  [OK] {filename}")
                            retry_crawl = False  # Thành công thì thoát vòng lặp retry

                        except Exception as e:
                            error_msg = str(e)
                            attempt += 1  # Tăng số lần thử lên
                ...
                print(f"\n[!] LỖI NGHIÊM TRỌNG ở danh sách trang {current_page}: {e}")
                print("[*] Tạm dừng 5 giây, khôi phục trang và thử lại...")
                time.sleep(5)
                try:
                    page.refresh()
...
if __name__ == "__main__":
    scrape_full_database(start_page=7)